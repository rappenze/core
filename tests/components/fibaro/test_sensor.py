...
async def test_energy_sensor_detected(
    hass: HomeAssistant,
    entity_registry: er.EntityRegistry,
    mock_fibaro_client: Mock,
    mock_config_entry: MockConfigEntry,
    mock_energy_sensor: Mock,
    mock_room: Mock,
) -> None:
    """Test that the attributes of the entity are correct."""
    # Arrange
    mock_fibaro_client.read_rooms.return_value = [mock_room]
    mock_fibaro_client.read_devices.return_value = [mock_energy_sensor]

    with patch("homeassistant.components.fibaro.PLATFORMS", [Platform.SENSOR]):
        # Act
        await init_integration(hass, mock_config_entry)
        # Assert
        entry = entity_registry.async_get("sensor.room_1_energy_meter_2")

        assert entry
        assert entry.unique_id == "hc2_111111.2"
        assert entry.original_name == "Room 1 Energy meter"
        assert entry.original_device_class == SensorDeviceClass.ENERGY
